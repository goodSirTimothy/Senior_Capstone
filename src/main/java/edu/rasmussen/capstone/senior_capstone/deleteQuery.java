...
public class deleteQuery extends AsyncTask<String,Void,String> {
    Context context;
    AlertDialog alertDialog;
    String android;

    deleteQuery(Context ctx) { context = ctx; }

    @Override
    protected String doInBackground(String... params) {
        String type = params[0];
        String login_url = "http://192.168.0.127:1234/mobileUsers.php";
        if(type.equals("info")) {
            try {
                android = params[1];
    ...
    public void onPostExecute(String result) {
        ...
        //call info.xml.
        if (result != ""){
            Intent info = new Intent(context.getApplicationContext(), deletelogin.class);
